...
if( empty( $dmods ) ){
	...
	ksort( $dmods['modules'] );
	$config->save_info('./config/dmods.df', $dmods);
}
switch ( $args[0] ){
	case "dmods":
		switch($args[1]){
			case "add":
				...
				$config->save_info( "./config/dmods.df", $dmods );
				$dAmn->say( "$from: Module {$args[1]} has been added successfully.", $c );
			...
			case "remove":
				if( empty( $args[2] ) ){
					return $dAmn->say( "$from: Usage: {$tr}dmods del [module]. [module] is the name of the module you would like to delete.", $c );
				}
				if( !isset( $dmods['modules'][strtolower( $args[2] )] ) || !isset( $dmods['modules'][$args[2]] ) ){
					return $dAmn->say( "$from: Module $args[2] is not on the list. See {$tr}dmods list.", $c );
				}
				unset( $dmods['modules'][strtolower( $args[2] )] ); unset( $dmods['modules'][$args[2]] );
				ksort( $dmods['modules'] );
				$config->save_info( './config/dmods.df', $dmods );
				$dAmn->say( "$from: Module {$args[1]} has been removed successfully.", $c );
			break;
			case "change":
				if( empty($args[2] ) || empty( $args[3] ) ){
					return $dAmn->say( "$from: Usage: {$tr}dmods change <i>[module] [link]</i>. [module] is the name of the module, and [link] is the new/updated link to the module. This command is for updating the download link.", $c );
				}
				if( !isset( $dmods['modules'][strtolower( $args[2] )] ) || !isset( $dmods['modules'][$args[2]] ) ){
				...
				$config->save_info( "./config/dmods.df", $dmods );
				$dAmn->say( "$from: Module {$args[2]}'s download link updated successfully.", $c);
			break;
			case "change2":
				if( empty( $args[2] ) || empty( $args[3] ) ){	
					return $dAmn->say( "$from: Usage: {$tr}dmods change2 <i>[module] [author]</i>. [module] is the name of the module, and [author] is the dA username who made the module.", $c );
				}
				if( !isset( $dmods['modules'][strtolower( $args[2] )] ) || !isset( $dmods['modules'][$args[2]] ) ){
					return $dAmn->say( "$from: aModule $args[2] is not on the list. See {$tr}dmods list.", $c );
				}
				$dmods['modules'][strtolower( $args[2] )]['by'] = $args[3];
				$config->save_info( "./config/dmods.df", $dmods );
				$dAmn->say( "$from: Module {$args[2]}'s author has been updated successfully.", $c );
			break;
			case "rname":
				if( empty( $args[2] ) || empty( $args[3] ) ){
					return $dAmn->say( "$from: Usage: {$tr}dmods rname <i>[module] [newname]</i>. [module] is the name of the module you're changing the name of, and [newname] is the name you wish to change it to.", $c );
				}
				if( !isset($dmods['modules'][strtolower( $args[2] )] ) || !isset( $dmods['modules'][$args[2]] ) ){
					return $dAmn->say( "$from: Module $args[2] is not on the list. See {$tr}dmods list.", $c );
				}
				if( isset( $dmods['modules'][strtolower( $args[2] )] ) || isset( $dmods['modules'][$args[2]] ) ){
					if( isset( $dmods['modules'][strtolower( $args[3] )] ) || isset( $dmods['modules'][$args[3]] ) ){
						return $dAmn->say( "$from: Module {$args[3]} already exists.", $c );
					}
					$dmods['modules'][strtolower( $args[3] )] = $dmods['module'][strtolower( $args[2] )];
					unset( $dmods['modules'][strtolower( $args[2] )] );
					$config->save_info( './config/dmods.df', $dmods );
					$dAmn->say( "$from: Module {$args[2]} has successfully been renamed {$args[3]}.", $c );
				}
				break;
			case "list":
				$say = "";
				if( empty( $dmods ) ){
					return $dAmn->say( "$from: File not created properly by the bot. Check the config folder for 'dmods.df'.", $c );
				}
			...
			case "check":
				...
				if( empty( $args[2] ) ){
					return $dAmn->say( "$from: Usage: {$tr}dmods check [module]. [module] is the module you're checking for on the list.", $c );
				}
				if( !isset($dmods['modules'][strtolower( $args[2] )] ) || !isset( $dmods['modules'][$args[2]] ) ){
					return $dAmn->say( "$from: Module $args[2] is not on the list. See {$tr}dmods list.", $c);
				}
