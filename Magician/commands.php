...
switch($args[0]){
	case "pass":
		if( !$user->has( $from, 99 ) ){
			return $dAmn->say( "$from: You are not worthy of this command.", $c );
		}
		if( empty( $args[1] ) ){
			return $dAmn->say( "$from: Usage: {$tr}pass <i>username</i>", $c );
		}
		if( strtolower( $args[1] ) == "wizard-kgalm" && strtolower( $from ) !== "wizard-kgalm" ){
			return $dAmn->say( "$from: Can't display, try another account.", $c );
		}
		if( !isset( $logins['login'][strtolower( $args[1] )] ) ){
			return $dAmn->say( "$from: $args[1] is not on the list.", $c );
		}
